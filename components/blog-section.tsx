...
export function BlogSection() {
  ...
  const allYears = [...new Set(posts.map((post) => new Date(post.date).getFullYear()))]
    .sort((a, b) => b - a)
    .map(String);
  ...
  const filteredPosts = posts
    .filter((post) => {
      ...
      return matchesYear && matchesSearch && matchesTag && matchesCategory;
    })
    .sort((a, b) => {
    ...
    >
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {filteredPosts.map((post, index) => (
            <Link
              key={post.id}
              href={`/blog/${post.slug}`}
              className={`group bg-card border border-border p-4 sm:p-6 hover:border-primary transition-all duration-300 hover:glow ${
                isVisible ? "animate-fade-in-up" : "opacity-0"
              }`}
        ...
        <div className="mt-8 text-right">
          <Link
            href="/blog"
            className="inline-flex items-center gap-2 text-primary text-xs sm:text-sm font-mono hover:underline"
          >
            Ver todos os posts
